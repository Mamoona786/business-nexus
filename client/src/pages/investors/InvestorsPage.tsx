...
export const InvestorsPage: React.FC = () => {
  ...
  const allStages = Array.from(
    new Set(investors.flatMap((i) => i.investmentStage || []).filter(Boolean))
  );
  const allInterests = Array.from(
    new Set(investors.flatMap((i) => i.investmentInterests || []).filter(Boolean))
  );
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-gray-600">Connect with investors who match your startup needs</p>
      </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-600">
                {loading ? 'Loading...' : `${investors.length} results`}
              </span>
            </div>
          </div>
